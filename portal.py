...
def home(request: Request):
    return request.app.state.templates.TemplateResponse(request=request, name='home.html', context={})
...
def dashboard(request: Request, db: Session = Depends(get_db)):
    ...
    if user.role == Role.APPLICANT.value:
        ...
        return request.app.state.templates.TemplateResponse(request=request, name='dashboard_applicant.html', context=ctx(request, user, apps=apps))
    if user.role == Role.COLLEGE_ADMIN.value:
        ...
        return request.app.state.templates.TemplateResponse(request=request, name='dashboard_college.html', context=ctx(request, user, apps=apps, counts=counts))
    if user.role == Role.COLLEGE_REVIEWER.value:
        ...
        return request.app.state.templates.TemplateResponse(request=request, name='dashboard_reviewer.html', context=ctx(request, user, assignments=assignments))
    if user.role in {Role.IRB_SECRETARIAT.value, Role.IRB_CHAIR.value, Role.SUPERADMIN.value}:
        ...
        return request.app.state.templates.TemplateResponse(request=request, name='dashboard_secretariat.html', context=ctx(request, user, apps=apps, pending_access=pending_access))
    return request.app.state.templates.TemplateResponse(request=request, name='dashboard_reviewer.html', context=ctx(request, user, assignments=[]))
...
def new_application_page(request: Request, db: Session = Depends(get_db)):
    ...
    return request.app.state.templates.TemplateResponse(request=request, name='application_new.html', context=ctx(request, user, colleges=colleges))
...
def application_detail(request: Request, app_id: str, db: Session = Depends(get_db)):
    ...
    return request.app.state.templates.TemplateResponse(request=request, name='application_detail.html', context=ctx(request,user,app=app,documents=documents,access_requests=access,assignments=assignments,reviewers=reviewers,can_docs=can_view_documents(db,user,app)))
